...
@section('content')
    ...
    <div class="container-fluid">
        <div style="float:right;" class="mb-5">
            <a href="{{ route('laporan_swakelola.create') }}" class="btn btn-primary">Tambah Data</a>
        </div>
...
<script>
    ...
    $(document).ready(function () {
        $('#laporan-table').DataTable({ // perhatikan huruf besar D
            processing: true,
            serverSide: true,
            ajax: "{{ route('laporan_swakelola-data') }}",
            columns: [
                { data: 'title' },
                { data: 'date' },
                { data: 'surveyor' },
                {
                    ...
                    render: function (data, type, row) {
                        return `<a href="/laporan_swakelola/${data}" class="btn btn-success">Detail</a> |
                                <a href="/laporan_swakelola/${data}/edit" class="btn btn-info">Edit</a> |
                                <button onclick="hapusUser(${data})" class="btn btn-danger">Hapus</button>
                                <form id="hapus-form-${data}" method="POST" action="/laporan_swakelola/${data}" style="display: none;">
                                    @csrf @method('DELETE')
                                </form>`;
                    }
                }
