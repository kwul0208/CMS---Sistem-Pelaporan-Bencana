...
class ReportTanggapDaruratBencanaController extends Controller
{
    ...
    public function data(Request $request) {
        ...
        return DataTables::of($query->get())
            ...
            ->addColumn('photo_1', fn($row) => $row->photo_1 ? '<a href="' . asset('storage/' . $row->photo_1) . '" target="_blank"><img src="' . asset('storage/' . $row->photo_1) . '" width="50" height="50"></a>' : '-')
            ->addColumn('photo_2', fn($row) => $row->photo_2 ? '<a href="' . asset('storage/' . $row->photo_2) . '" target="_blank"><img src="' . asset('storage/' . $row->photo_2) . '" width="50" height="50"></a>' : '-')
            ->addColumn('photo_3', fn($row) => $row->photo_3 ? '<a href="' . asset('storage/' . $row->photo_3) . '" target="_blank"><img src="' . asset('storage/' . $row->photo_3) . '" width="50" height="50"></a>' : '-')
            ->addColumn('photo_4', fn($row) => $row->photo_4 ? '<a href="' . asset('storage/' . $row->photo_4) . '" target="_blank"><img src="' . asset('storage/' . $row->photo_4) . '" width="50" height="50"></a>' : '-')
            ->addColumn('photo_5', fn($row) => $row->photo_5 ? '<a href="' . asset('storage/' . $row->photo_5) . '" target="_blank"><img src="' . asset('storage/' . $row->photo_5) . '" width="50" height="50"></a>' : '-')
